...
for r in range(1, min(13, ws.max_row+1)):
    texts = [(template_filler_module._normalize(ws.cell(row=r, column=c).value)) for c in range(1, ws.max_column+1)]
    print(r, texts)
...
hdr_row, hdr_map = template_filler_module.detect_header_row(ws, search_start=1, search_end=min(12, ws.max_row))
